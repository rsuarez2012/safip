...
<div class="box-body">
    ...
    <table class="display table table-hover table-responsive" id="tabla_pacientes">
        ...
        @foreach ($operadores as $operador)
            <tr role="row" class="odd">
                <td>@if(!empty($operador->empresas->nombre))
                        {{$operador->empresas->nombre}}
                    @else
                ...
                <td>
                    <button class="label boton-dato label-primary" id="boton-dato" name="boton-dato"
                            value="{{$operador->rif}}Ç{{$operador->nombre}}Ç{{$operador->email}}">Capturar Email
                    </button>
                </td>
            </tr>
        @endforeach
    ...
    <?php
    echo str_replace('/?', '?', $operadores->render());
    }else{
    ?>

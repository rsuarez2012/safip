  	...
  	<div class="modal-dialog" role="document">
    	<div class="modal-content"   style="width: 300px;margin: auto;">
      		<div class="modal-header">
        		<h3  class="modal-title" style="display: inline;"><i class="fa fa-truck"></i> Seleccione Proveedor</h3>
      		...
      		<div class="modal-body text-center" >
        		<form style="display: inline;" action="{{ route('manageCotizacionPaquete-qantu-A') }}" method="get">
          			<input type="hidden" name="cotizacion" id="cot_paquete_id">
          			<button type="submit" class="btn btn-danger">Qantu Travel</button>
        		</form>
        		<form style="display: inline;" action="{{ route('venta.proveedor.index') }}" method="get">
          			<input type="hidden" name="proveedor" id="vendedor">
          			<button type="submit" class="btn btn-danger">Otro Proveedor</button>
        		</form>
      		</div>
      		<div class="modal-footer">
        		<button type="button" class="pull-right btn btn-secondary" @click="modalHide('modalProcesar')"><i class="fa fa-close"></i> Cancelar</button>
      		</div>
    	</div>
  	</div>
</div>
<div class="row">                        			
	...
	<div class="col-sm-6">
		...
		<!--<a class="btn btn-danger pull-right" type="submit" id="destino-seleccionado">Agregar</a>-->
		...
		<table class="table table-bordered" id="destinos-select" >
			...
			<tbody>
				...
				<tr class="destino_id{{ $destino->id }}">
					<td style="text-align: center;">{{$destino->destino->nombre}}</td>
					...
					<td style="text-align: center;">
						...
						<a type="button" class="btn btn-warning btn-xs" id="edit-dest" data-id="{{ $destino->noches->id }}" data-toggle="tooltip" data-original-title="Editar Dias"><i class="fa fa-pencil"></i></a>
						<a type="button" class="btn btn-danger btn-xs delete-dest" id="delete-dest" data-id="{{ $destino->id }}" data-toggle="tooltip" data-original-title="Eliminar Destino"><i class="fa fa-trash"></i></a>
	...
	<div class="col-sm-6" style="display: block;" id="dest-hot">
		<div class="form-group">
			...
			<select class="form-control desti" name="destino" id="hot-dest" style="width: 100%">
				<option value=''>Seleccione el destino</option>
				@foreach($destinosP as $destino)
					<option value='{{ $destino->destino_id }}' data-id="{{ $destino->destino_id }}">{{ $destino->destino->nombre }}</option>
...
<div class="row">
	<div class="col-sm-12">
		...
		<!--<a class="btn btn-danger pull-right" type="submit" id="enlazar" style="display: none;">Enlazar</a>-->
		<a id="btn-step2-hoteles" class="btn btn-danger pull-right" style="display: none" type="submit">Enlazar</a>
			<a href="{{route('paquete.edit.paso3',$paquete)}}" class="btn btn-danger pull-right" id="next">Siguiente</a>
	</div>
</div>
{{--@section('script')
